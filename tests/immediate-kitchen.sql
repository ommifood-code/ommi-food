...
begin
 ...
 offer:=public.meal_offer_create(token,0,jsonb_build_object('serves',1,'ingredients','خضر','fulfilment_type','pickup','pickup_instructions','عنوان اختبار خاص'));
 ...
 response:=public.place_meal_order(offer,1,('{"name":"زبون اختبار","phone":"0600000082"}'::jsonb||jsonb_build_object('requested_at',now()+interval '2 days')),'pickup',repeat('8',64));
 ...
 blocked:=false;begin perform public.place_meal_order(offer,1,('{"name":"زبون آخر","phone":"0600000083"}'::jsonb||jsonb_build_object('requested_at',now()+interval '2 days')),'pickup',repeat('9',64));exception when others then blocked:=true;end;
 if not blocked then raise exception 'FAIL suspended booking';end if;
 execute 'reset role';
end $$;
rollback;
select 'PASS: registration active without fake verification, anon discovery and ordering, pending until chef accepts, idempotent cancellation, suspension; rolled back' as result;
