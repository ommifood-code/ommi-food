begin;
do $$
declare r jsonb;c uuid;t text;other_t text;oid uuid;offer uuid;req uuid:=gen_random_uuid();d jsonb:='{"name":"كسكس اختبار","price":40}';p jsonb;n integer;blocked boolean;
begin
 ...
 p:=jsonb_build_object('specialty','أكلات تقليدية وشعبية','portion','شخص واحد','ingredients','قمح وخضر','quantity',2,'order_until',now()+interval '1 day','ready_at',now()+interval '2 days','fulfilment_type','pickup','pickup_instructions','عنوان خاص للاختبار');
 ...
 r:=public.place_meal_order(offer,2,'{"name":"زبون الاختبار","phone":"0600000092"}','pickup',repeat('c',64));
 ...
 if (select allocated from public.meal_offers where id=offer)<>2 then raise exception 'FAIL allocation';end if;
 ...
 if (select allocated from public.meal_offers where id=offer)<>0 then raise exception 'FAIL release';end if;
...
rollback;
select 'PASS: private/coarse location, session, city, atomic publish, idempotent retry, pending order, allocation, cancellation, suspension; fixtures rolled back' result;
