...
begin
 ...
 offer:=public.meal_offer_create('temporary-test-session',0,jsonb_build_object('portion','حصة لشخص واحد','ingredients','قمح وخضر','allergens','يحتوي القمح','quantity',2,'order_until',now()+interval '1 day','ready_at',now()+interval '2 days','fulfilment_type','both','delivery_fee',15,'delivery_areas',jsonb_build_array('معاريف'),'pickup_instructions','عنوان اختبار خاص'));
 ...
 if response<>public.place_meal_order(offer,2,'{"name":"زبون اختبار","phone":"0600000002","area":"معاريف","address":"عنوان الاختبار"}','delivery',t) then raise exception 'FAIL retry idempotency'; end if;
 ...
 blocked:=false;begin perform public.place_meal_order(offer,1,'{"name":"زبون ثالث","phone":"0600000004","area":"حي غير مخدوم","address":"عنوان الاختبار"}','delivery',t3);exception when others then blocked:=true;end;
 if not blocked then raise exception 'FAIL delivery area validation';end if;
 update public.meal_offers set order_until=now()-interval '1 minute' where id=offer;
 blocked:=false;begin perform public.place_meal_order(offer,1,'{"name":"زبون ثالث","phone":"0600000004"}','pickup',t3);exception when others then blocked:=true;end;
 if not blocked then raise exception 'FAIL expired offer';end if;
