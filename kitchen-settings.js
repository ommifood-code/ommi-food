...
function kitchenSettingsFields(settings = {}) {
  const selected = String(settings.work_days || "")
    .split("،")
    .map((x) => x.trim());
...
function bindKitchenSettings(form, settings = {}) {
  const mode = form.elements.fulfilment_type;
  mode.value = settings.fulfilment_type || "pickup";
  ...
  const sync = () => {
    const delivery = mode.value !== "pickup",
      pickup = mode.value !== "delivery";
    form.querySelector("[data-delivery]").hidden = !delivery;
    form.querySelector("[data-pickup]").hidden = !pickup;
    form.elements.delivery_areas.required = delivery;
    form.elements.delivery_fee.required = delivery;
    form.elements.pickup_instructions.required = pickup;
  };
  mode.onchange = sync;
  sync();
  form.querySelector("[data-all-days]").onclick = () =>
    form.querySelectorAll("[name=work_day]").forEach((x) => (x.checked = true));
}
function readKitchenSettings(form) {
  return {
    work_days: [...form.querySelectorAll("[name=work_day]:checked")].map(
      (x) => x.value,
    ),
    fulfilment_type: form.elements.fulfilment_type.value,
    pickup_instructions: form.elements.pickup_instructions.value.trim(),
    delivery_areas: [
      ...new Set(
        form.elements.delivery_areas.value
          .split("\n")
          .map((x) => x.trim())
          .filter(Boolean),
      ),
    ],
    delivery_fee: Number(form.elements.delivery_fee.value),
  };
}
async function openKitchenPreferences() {
  const c = await restoreChefSession(false);
  if (!c) throw Error("ادخل إلى مطبخك مجددًا.");
  const settings = await mealRpc("chef_order_settings", {
    p_session_token: chefSessionToken,
  });
  const screen = mealPanel("kitchenPreferences", "أيام العمل والاستلام"),
  ...
  bindKitchenSettings(form, settings);
  form.onsubmit = (e) => {
    e.preventDefault();
    if (!form.reportValidity()) return;
    mealBusy(form.querySelector("[type=submit]"), async () => {
      await mealRpc("chef_order_settings", {
        p_session_token: chefSessionToken,
        p_settings: readKitchenSettings(form),
      });
      markFormSaved(form);
      showToast("حُفظت إعدادات مطبخك لجميع الطلبات الجديدة.");
...
function kitchenDaysLabel(days) {
  return days ? `أيام العمل المعتادة: ${days}` : "أيام العمل تُتفق مع المطبخ";
}
