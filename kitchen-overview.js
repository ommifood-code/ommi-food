...
async function loadKitchenOverview(c){
 ...
 offers.forEach(o=>{const card=document.createElement('article');card.className='kitchen-dish-card';const photo=safeImageUrl(o.image_url);card.innerHTML=`${photo?`<img src="${escapeHtml(photo)}" alt="${escapeHtml(o.dish_name)}" loading="lazy">`:'<div class="dish-placeholder" role="img" aria-label="Ommi Food — لم تُضف صورة للطبق"><span class="placeholder-brand" dir="ltr">Ommi Food</span><span class="placeholder-caption">من مطابخ أمهاتنا</span></div>'}<div class="dish-card-body"><h3>${escapeHtml(o.dish_name)}</h3><strong class="dish-price">${o.reference_price!=null?mealMoney(o.reference_price)+' — سعر مرجعي للشخص':'حدّد السعر المرجعي للشخص'}</strong><p>${o.group_pricing?'سعر المجموعات قابل للاتفاق':'من الأطباق التي نتقنها'}</p><span class="dish-state ${o.active?'is-available':'is-paused'}">${o.active?'متاح للطلب':'متوقف مؤقتًا'}</span></div>`;const controls=document.createElement('div');controls.className='dish-card-controls';controls.append(mealAction('تعديل الطبق',()=>openMealOfferForm(o)),mealAction(o.active?'إيقاف مؤقت':'إتاحة الطبق',async()=>{await mealRpc('meal_offer_availability',{p_session_token:chefSessionToken,p_offer_id:o.id,p_active:!o.active});await refresh();}));card.append(controls);dishes.append(card);});
 }
 if(results[1].status==='rejected'){ordersBox.textContent='تعذر تحميل الطلبات. اضغط تحديث للمحاولة مجددًا.';return;}
 const orders=results[1].value||[],priority={pending:0,discussing:1,proposed:1,accepted:2,preparing:3,ready:4};orders.sort((a,b)=>(priority[mealEffectiveStatus(a)]??4)-(priority[mealEffectiveStatus(b)]??4));
