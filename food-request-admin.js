...
const requestAdminTime = (value) => OrderRules.date(value, "—");
function requestAdminOverdue(o) {
  return OrderRules.isOverdue(o);
}
...
function requestAdminStatus(o) {
  ...
  const labels = {
    ...
    discussing: "قبله المطبخ — بانتظار الاتفاق",
    proposed: "تفاصيل سابقة — تحتاج متابعة",
    accepted: "تم الاتفاق",
    preparing: "قيد التحضير",
    ready: "جاهز",
    delivered: "المطبخ أعلن التسليم",
    rejected: "لم يتم الاتفاق",
    cancelled: "ملغى",
  };
  return requestAdminOverdue(o)
    ? "فات الموعد المطلوب"
    : labels[o.status] || o.status || "غير محدد";
}
function requestAdminStats(m) {
  const stats = [
    ...
    [
      "متوسط الرد",
      m.average_response_minutes == null
        ? "—"
        : `${m.average_response_minutes} دقيقة`,
    ],
  ...
  if (grid)
    grid.innerHTML = stats
      .map(
        ([label, value]) =>
          `<div class="admin-stat"><strong>${e(value)}</strong><span>${e(label)}</span></div>`,
      )
      .join("");
  const note = document.getElementById("adminStatsNote");
  if (note)
    note.textContent =
      "القيمة تشمل الأسعار المسجّلة والمؤكدة داخل التطبيق فقط. أثمان الاتفاقات الهاتفية غير المسجّلة غير محسوبة؛ ليست هذه مداخيل المنصة.";
}
...
async function loadRequestAdmin() {
  ...
  if (error) {
    section.innerHTML =
      '<p class="admin-load-error">تعذر تحميل طلبات الزبائن. حدّث الصفحة للمحاولة مجددًا.</p>';
    return;
  }
  ...
  window.setAdminAttentionCounts?.("requests", {
    urgent: requests.filter(urgent).length,
    complaints: requests.filter(openComplaint).length,
  });
  ...
  if (!requests.length) {
    cards.innerHTML = '<p class="admin-empty">لا توجد طلبات زبائن بعد.</p>';
    window.applyAdminView?.();
    return;
  }
  requests.sort(
    (a, b) =>
      Number(urgent(b)) - Number(urgent(a) || 0) ||
  ...
  for (const o of requests) {
    ...
    card.className =
      "meal-card admin-request-card" +
      (urgent(o) ? " admin-priority-urgent" : "") +
    ...
    const agreement = o.chef_agreed_at
      ? `<div class="admin-agreement"><strong>المطبخ سجّل الاتفاق بعد التواصل</strong><p>الموعد: ${o.agreed_at ? requestAdminTime(o.agreed_at) : "حسب الاتفاق"}</p></div>`
      : o.agreement_version
        ? `<div class="admin-agreement"><strong>${o.customer_agreed_at ? "الاتفاق الذي وافق عليه الزبون" : "تفاصيل سابقة مقترحة"}</strong><p>${e(o.agreed_dish || o.dish_name)} · ${e(o.agreed_people || "—")} أشخاص · ${adminMoney(o.agreed_total)} · ${requestAdminTime(o.agreed_at)}</p><p>${e(o.agreed_fulfilment || "")}</p></div>`
        : "";
    card.innerHTML = `<div class="admin-request-heading"><h3>${dish} <small>— ${ref}</small></h3><strong class="admin-status ${urgent(o) ? "late" : ""}">${e(currentStatus)}</strong></div><p>المطلوب: ${e(o.people ?? "—")} أشخاص · الموعد: ${o.requested_at ? requestAdminTime(o.requested_at) : "حسب الاتفاق"}</p><p>التقدير: ${adminMoney(o.estimate)}${o.offer_id ? "" : " · طبق طلبه الزبون خارج القائمة"}</p><p>الزبون: ${e(o.customer_name || "—")} — <a href="tel:${e(o.customer_phone || "")}">${e(o.customer_phone || "غير متوفر")}</a></p><p>المطبخ: ${e(o.chef_phone || "غير متوفر")}</p>${overdue(o) ? '<div class="admin-warning"><strong>تأخر الطلب.</strong> اتصل بالمطبخ والزبون لتثبيت موعد جديد أو إنهاء الطلب.</div>' : ""}${agreement}<p>إعلان التسليم: ${o.chef_delivered_at ? requestAdminTime(o.chef_delivered_at) : "لم يسجّل"} · تأكيد الزبون: ${o.received_at ? requestAdminTime(o.received_at) : "غير معروف"}</p><p>التقييم: ${o.rating ?? "لم يقيّم"}${o.feedback ? ` — ${e(o.feedback)}` : ""}</p>${o.reason ? `<p>سبب الاعتذار أو الإلغاء: ${e(o.reason)}</p>` : ""}${o.notes ? `<p>رغبة الزبون: ${e(o.notes)}</p>` : ""}${o.complaint ? `<div class="admin-complaint"><strong>بلاغ ${o.complaint_resolved_at ? "مراجع" : "مفتوح"}:</strong> ${e(o.complaint)}</div>` : ""}`;
    const action = (label, name) => {
      ...
      b.onclick = async () => {
        b.disabled = true;
        try {
          const r = await db.rpc("food_request_admin", {
            p_id: o.id,
            p_action: name,
          });
          if (r.error) throw r.error;
          await loadRequestAdmin();
        } catch {
          toastMsg("تعذر حفظ المتابعة.");
        } finally {
          b.disabled = false;
        }
      };
    ...
    if (
      ...
      card.insertAdjacentHTML(
        "beforeend",
        `<p><strong>ينتظر منذ ${requestAdminElapsed(o.created_at)}</strong>${o.admin_contacted_at ? ` · آخر متابعة: ${requestAdminTime(o.admin_contacted_at)}` : ""}</p>`,
      );
...
const requestAdminRefresh = () =>
  loadAdminOrders().catch(() => toastMsg("تعذر تحديث الطلبات."));
document
  .getElementById("adminRequestsRefresh")
  ?.addEventListener("click", requestAdminRefresh);
document
  .getElementById("adminStatsRefresh")
  ?.addEventListener("click", requestAdminRefresh);
