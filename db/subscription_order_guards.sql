...
begin
 ...
 if not public.chef_subscription_valid(v_id) or not o.active or o.order_until<=now() or not exists(select 1 from public.chefs where id=v_id and status='active' and phone_verified and membership_status='active' and membership_type in ('honorary','paid')) then raise exception 'offer unavailable'; end if;
