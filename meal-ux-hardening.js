...
mountMealDashboard=function(c){
 ...
 if(!guide){
  ...
  guide.innerHTML=`<div><strong>أطباقي المحفوظة</strong><span>وصفات وأسعار تحفظها في مطبخك ويمكنك تعديلها في أي وقت. حفظ الطبق وحده لا يجعله متاحًا للطلب.</span></div><div><strong>وجبات الحجز</strong><span>عندما تريد بيع طبق في موعد محدد، أتحه كوجبة وحدد عدد الحصص وآخر أجل للطلب وموعد الجاهزية.</span></div>`;
...
openMealOfferForm=async function(c){
 ...
 box.prepend(uxInfo('offer-meaning','أنت لا تنشئ طبقًا جديدًا هنا','أنت تختار طبقًا محفوظًا في مطبخك وتحدد متى سيكون متاحًا للحجز وكم حصة ستبيع. السعر يؤخذ من الطبق المحفوظ.'));
};
...
openMealOrdering=async function(chef){
 ...
 if(!note){note=uxInfo('pending-order-warning','طلبك ينتظر قبول المطبخ','إرسال الطلب لا يعني تأكيده. لا تُحجز الحصص إلا عندما يقبل المطبخ الطلب. تابع الحالة من «طلباتي» واضغط «تحديث» لرؤية آخر تغيير.');fields.prepend(note);}
};
...
if(typeof openChefMealOrders==='function'){
 ...
 openChefMealOrders=async function(){
  ...
  if(!note){note=uxInfo('chef-order-rule','متى تُحجز الحصص؟','الطلب المعلّق لا يحجز حصصًا. الحصص تُخصم عند القبول، وإذا أُلغي طلب مقبول يعيد النظام حصصه إلى الوجبة.');box.prepend(note);}
 };
}
