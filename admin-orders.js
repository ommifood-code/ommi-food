...
async function loadAdminOrders(){
 const version=++adminOrdersVersion,box=document.getElementById('adminOrdersList');if(!box.children.length)box.textContent='جاري تحميل الطلبات...';
 ...
 const contacted=document.createElement('button');contacted.className='admin-secondary';contacted.textContent='سجل أنني اتصلت بالمطبخ';contacted.onclick=async()=>{contacted.disabled=true;try{const result=await db.rpc('admin_contacted_meal_order',{p_order_id:o.id});if(result.error)throw result.error;await loadAdminOrders();}catch{toastMsg('تعذر تسجيل الاتصال.');}finally{contacted.disabled=false;}};card.append(contacted);
 }
